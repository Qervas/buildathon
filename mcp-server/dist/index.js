...
async function pollUntilDone(jobId, timeoutMs = 180_000) {
...
server.tool("generate_animation", "Generate a skeletal animation from a text description. Returns a BVH file URL with 77-joint SOMA skeleton at 30fps. Powered by NVIDIA Kimodo.", {
...
server.tool("list_animations", "List previously generated animations from the gallery. Returns completed sessions with download links. Use this to check past results without re-generating.", {}, async () => {
    const sessions = (await apiGet("/api/sessions"));
    if (!sessions.length) {
        return { content: [{ type: "text", text: "No animations generated yet." }] };
    }
    const lines = ["# Generated Animations\n"];
...
server.tool("get_animation_status", "Check the status of a running animation generation job. Use this if generate_animation timed out or you want to check a job later.", {
